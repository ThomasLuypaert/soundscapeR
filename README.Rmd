...
```{r Fig_1, fig.align = 'center', out.width = "75%", fig.cap = "The benefits of Passive Acoustic Monitoring over conventional active acoustic sampling by an in-situ observer.", echo=FALSE}
...
```{r Fig_4, fig.align = 'center', out.width = "100%", fig.cap = "A theoretical representation of long-duration eco-acoustic data collection. Data is collected over a 7-day acoustic survey period using a continuous sampling regime. The sound files are grouped per 24-hour period and considered a sample of the soundscape.", echo=FALSE}
knitr::include_graphics(here::here("man/figures/Fig_1_Ecoacoustic_data.png"))
```
$$\\[0.001in]$$
***Fig 4:*** *A theoretical representation of long-duration eco-acoustic data collection. Data is collected over a 7-day acoustic survey period using a continuous sampling regime. The sound files are grouped per 24-hour period and considered a sample of the soundscape.*
...
```{r Fig_5, fig.align = 'center', out.width = "100%", fig.cap = "A theoretical representation of how spectral indices are computed. First, each 1-minute sound file is subjected to a Fast Fourier Transformation, which extracts information on the amplitude variation across the time-frequency domain. Next, the resulting spectrogram is subjected to a noise-removal step. Finally, a mathematical equation (spectral index) is applied to the amplitude values in each frequency bin, resulting in a spectral index vector with one index value per frequency bin.", echo=FALSE}
knitr::include_graphics(here::here("man/figures/Fig_2_Index_computation.png"))
```
$$\\[0.001in]$$
***Fig 5:*** *A theoretical representation of how spectral indices are computed. First, each 1-minute sound file is subjected to a Fast Fourier Transformation, which extracts information on the amplitude variation across the time-frequency domain. Next, the resulting spectrogram is subjected to a noise-removal step. Finally, a mathematical equation (spectral index) is applied to the amplitude values in each frequency bin, resulting in a spectral index vector with one index value per frequency bin.*
...
```{r Fig_6, fig.align = 'left', out.width = "100%", fig.cap = "A theoretical representation of the chronological concatenation step. The all spectral index vectors resulting from the index computation step are merged chronologically, resulting in a time-by-frequency data frame containing the spectral index values.", echo=FALSE}
knitr::include_graphics(here::here("man/figures/Fig_3_Chronological_concatenation.png"))
```
$$\\[0.001in]$$
***Fig 6:*** *A theoretical representation of the chronological concatenation step. The all spectral index vectors resulting from the index computation step are merged chronologically, resulting in a time-by-frequency data frame containing the spectral index values. On the right, a visual representation of the spectral index values in the time-by-frequency data frame for seven 24-hour samples of the soundscape.*
...
```{r Fig_8, fig.align = 'center', out.width = "100%", fig.cap = "A visualization showing the binarization step, starting from the chronologically concatenated CVR-index values for a 24-hour soundscape sample. Next, a threshold value is applied, which is derived using a binarization algorithm. Based on this threshold value, the raw CVR-values are transformed into binary detection (1) / non-detection (0) values.", echo=FALSE}
knitr::include_graphics(here::here("man/figures/Fig_4_binarization.png"))
```
$$\\[0.001in]$$
***Fig 8:*** *"A visualization showing the binarization step, starting from the chronologically concatenated CVR-index values for a 24-hour soundscape sample. Next, a threshold value is applied, which is derived using a binarization algorithm. Based on this threshold value, the raw CVR-values are transformed into binary detection (1) / non-detection (0) values."*

...
#' @param twilight A character string of the twilight method to be used for sunrise and sunset as the boundary between day and night. Options can be found in the \code{\link[photobiology]{day_night}} documentation.
...
sounddiv_profile_internal <- function(df, qvalues=seq(0, 5, 0.1), type="total",
  ...
  if (mintime=="default"){
    time1 <- min(as.POSIXct(strptime(paste(date, colnames(df), sep=" "),
                                     format= "%Y-%m-%d %H:%M:%S",
                                     tz=tz)))
  }
  ...
  if (maxtime=="default"){
    time2 <- max(as.POSIXct(strptime(paste(date, colnames(df), sep=" "),
                                     format= "%Y-%m-%d %H:%M:%S",
                                     tz=tz)))
  }
  ...
  colnames_df <- as.POSIXct(strptime(paste(date, colnames(df), sep=" "),
                                     format= "%Y-%m-%d %H:%M:%S",
                                     tz=tz))
...
#'
#' @return A soundscape diversity profile plot or table.
sounddiv_profile_point_internal <- function(df, qvalues=seq(0, 5, 0.1), type="total",
...
sounddiv_prof <- function(df, qvalues=seq(0, 5, 0.1), type="total",
                                   minfreq="default", maxfreq="default",
                                   mintime="default", maxtime="default",date, lat, lon,
                                   twilight="sunlight", dawnstart=0, dawnend=5400,
                                   duskstart=5400, duskend=0, display="plot",
                                   output="percentage"){

  if (class(df)=="data.frame"){
    sounddiv_profile <- sounddiv_profile_internal(df=df, qvalues=qvalues, type=type,
    ...
    sounddiv_profile_point <- sounddiv_profile_point_internal(df=df,
                                                              qvalues=qvalues,
                                                              type=type,
                                                              minfreq=minfreq,
                                                              maxfreq=maxfreq,
    ...
    if (display=="plot"){
      ...
      else{errorCondition(message = "Invalid display argument - please consult package documentation for options")}
    }
  }
    ...
    if (class(df)=="list"){
      ...
      for (i in 1:length(df)){
        sounddiv_profile[[i]] <- sounddiv_profile_internal(df=df[[i]],
                                                           qvalues=qvalues,
                                                           type=type,
                                                           minfreq=minfreq,
                                                           maxfreq=maxfreq,
        ...
        sounddiv_profile_point[[i]] <- sounddiv_profile_point_internal(df=df[[i]],
                                                                 qvalues=qvalues,
                                                                 type=type,
                                                                 minfreq=minfreq,
                                                                 maxfreq=maxfreq,
      ...
      names(sounddiv_profile) <- names(df)
      names(sounddiv_profile_point) <- names(df)
    ...
    else{errorCondition(message = "Invalid class argument - please consult package documentation for options")}
  }
}

...
pair_dis_modified <- function(countable,qvalue,tree,hierarchy,metric){
  ...
  gamma_div_fast <- function(otutable,qvalue,tree){
    ...
    if(dim(otutable)[1] < 2) stop("The OTU table only less than 2 OTUs")
    if(dim(otutable)[2] < 2) stop("The OTU table contains less than 2 samples")
    if(sum(colSums(otutable)) != ncol(otutable)){otutable <- hilldiv::tss(otutable)}
    if(missing(qvalue)) stop("q value is missing")
    if(qvalue < 0) stop("q value needs to be possitive (equal or higher than zero)")
  ...
  levels <- paste("L",seq(1, leveln, 1),sep="")
  if(!missing(hierarchy)){
  ...
  if(leveln > 1){
    ...
    for(i in c(2:leveln)){
      ...
      countable.sub <- stats::aggregate(subset(countable.sub, select=rownames(countable)), by=list(countable.sub[,as.character(levels[i])]), FUN=mean)
      ...
      for (y in indices){
        if(is.na(beta.matrix[x,y])){ #to avoid repeating mirror operations
          ...
          if(identical(x,y) == TRUE){
            ...
            if(missing(tree)){
              alpha <- hilldiv::alpha_div(combination,qvalue)
              gamma <- hilldiv::gamma_div(combination,qvalue)
            }else{
            ...
            if('C' %in% metric){
              CqN.matrix[y,x] <- hilldiv::beta_dis(beta=beta,qvalue=qvalue,N=2,metric="C",type="dissimilarity")$CqN
            }

            if('U' %in% metric){
              UqN.matrix[y,x] <- hilldiv::beta_dis(beta=beta,qvalue=qvalue,N=2,metric="U",type="dissimilarity")$UqN
            }

            if('V' %in% metric){
              VqN.matrix[y,x] <- hilldiv::beta_dis(beta=beta,qvalue=qvalue,N=2,metric="V",type="dissimilarity")$VqN
            }

            if('S' %in% metric){
              SqN.matrix[y,x] <- hilldiv::beta_dis(beta=beta,qvalue=qvalue,N=2,metric="S",type="dissimilarity")$SqN
            }

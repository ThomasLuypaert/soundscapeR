...
#'
#' @details The \code{ss_index_config()} function is normally only used internally
#' by the \code{ss_index_calc()} function, although it is possible for the
...
#'
#' @export
ss_index_config <- function(samplerate = 41000, window = 256) {
...
#' @export
#'
ss_index_calc <- function(fileloc,
                       outputloc = NULL,
                       samplerate = 44100,
                       window = 256,
                       parallel = FALSE) {
  ...
  ss_index_config(samplerate = samplerate, window = window)

...
#'
#' @param aggregated_soundscape_list A list of aggregated_soundscape objects. Each object in the
#' list should be produced using the \code{\link{ss_aggregate}} function.
#'
#' @param grouping A numeric or character vector indicating potential grouping of the elements in the 'aggregated_soundscape_list' object.
#' Make sure that the grouping vector has the same length as the 'aggregated_soundscape_list' argument.
#'
...
ss_pcoa <- function(aggregated_soundscape_list,
                    grouping = NA,
                    screeplot = FALSE){

  # 0. Testing

  # 0.1. Check that the aggregated_soundscape_list argument
  ...
  assertthat::assert_that(test_1(aggregated_soundscape_list))
  ...
  assertthat::assert_that(test_2(aggregated_soundscape_list))
  ...
  assertthat::on_failure(test_3) <- function(call, env){
    ...
    paste0(deparse(call$x), " is not a list of S4-objects of the type 'soundscape'. The list may contain different objects or empty elements. Please supply a list of aggregated_soundscape objects produced by the ss_aggregate() function. Consult the package documentation for further information.")

  }

  assertthat::assert_that(test_3(aggregated_soundscape_list))
  ...
  assertthat::assert_that(test_4(aggregated_soundscape_list))
  assertthat::assert_that(test_5(aggregated_soundscape_list))
  ...
  assertthat::on_failure(test_8) <- function(call, env){
    ...
    paste0(deparse(call$x), " contains samplerate arguments that are not a single positive integer. Did you supply the aggregated_soundscape argument produced using the ss_aggregate() function? If so, something has gone wrong, please re-run the ss_binarize() and ss_aggregate() functions, and pay special attention to the samplerate and window arguments.")

  }

  assertthat::on_failure(test_9) <- function(call, env){

    paste0(deparse(call$x), " contains window arguments that are not a single positive integer. Did you supply the aggregated_soundscape argument produced using the ss_aggregate() function? If so, something has gone wrong, please re-run the ss_binarize() and ss_aggregate() functions, and pay special attention to the samplerate and window arguments.")

  }

  assertthat::assert_that(test_8(aggregated_soundscape_list))
  assertthat::assert_that(test_9(aggregated_soundscape_list))
  ...
  assertthat::assert_that(test_10(aggregated_soundscape_list))
  ...
  assertthat::on_failure(test_11) <- function(call, env){
    ...
    paste0(deparse(call$x), " contains threshold arguments that are not a single numeric value. Did you supply the aggregated_soundscape argument produced using the ss_aggregate() function? If so, something has gone wrong, please re-run the ss_binarize() and ss_aggregate() functions, and pay special attention to the value argument is you're supplying a custom threshold value.")

  }

  assertthat::assert_that(test_11(aggregated_soundscape_list))
  ...
  assertthat::assert_that(test_12(aggregated_soundscape_list))
  ...
  assertthat::on_failure(test_13) <- function(call, env){
    ...
    paste0(deparse(call$x), " contains invalid merged_df, binarized_df, or aggregated_df data frames. It is possible the argument is not a data frame, is empty, or contains NA/non-numeric values. Did you supply the aggregated_soundscape argument produced using the ss_aggregate() function? If so, something has gone wrong, please re-run the ss_aggregate() function.")
  ...
  assertthat::assert_that(test_13(aggregated_soundscape_list))
  assertthat::assert_that(test_14(aggregated_soundscape_list))
  assertthat::assert_that(test_15(aggregated_soundscape_list))
  ...
  assertthat::assert_that(test_14(aggregated_soundscape_list))
  assertthat::assert_that(test_15(aggregated_soundscape_list))
  assertthat::assert_that(test_16(aggregated_soundscape_list))
  ...
  assertthat::on_failure(test_18) <- function(call, env){
    ...
    paste0(deparse(call$x), " does not have the same length as aggregated_soundscape_list. Please supply the grouping argument as a vector of group names of the same length as the aggregated_df_list")
  ...
  # 1. Check that the aggregated_soundscape object contain the 'raw' incidences, not the 'incidence_freq'

  if(all(sapply(aggregated_soundscape_list, function(x) x@output == "raw")) == FALSE){

    wrong_output <- which(sapply(aggregated_soundscape_list,
                                 function(x) x@output == "raw") == FALSE)

    for (i in 1:length(wrong_output)){

      aggregated_soundscape_list[[wrong_output[i]]]@aggregated_df <- aggregated_soundscape_list[[wrong_output[i]]]@aggregated_df * aggregated_soundscape_list[[wrong_output[i]]]@effort_per_time[1]
      aggregated_soundscape_list[[wrong_output[i]]]@output <- "raw"
...
site_by_OSU_matrix <-
  as.data.frame(
    do.call(what = rbind,
            args = lapply(aggregated_soundscape_list,

...
CVR_computation <- function(file, window = 256, theta = 3){
  ...
  dB_mode_per_row <- function(x){
    ...
    sound_hist <- hist(as.numeric(x), breaks = seq.100, plot = FALSE)
  ...
  if(parallel == TRUE){
    ...
    CVR_list <- foreach::foreach(i = 1:n_files, .packages = c("tuneR", "seewave", "soundscapeR"),
                                 .options.snow = opts) %dopar% {
                                   ...
                                   write.csv(x = t(as.data.frame(ale_spectro_wide$CVR_index)),file = paste0(output_path, "/", gsub(x = basename(file_list_new[i]),pattern = ".wav", replacement = "") , "_CVR.csv"))
    ...
    for(j in 1:length(file_list_new)){
      ...
      write.csv(x = t(as.data.frame(CVR_list[[j]])),file = paste0(output_path, "/", gsub(x = basename(file_list_new[j]),pattern = ".wav", replacement = "") , "_CVR.csv"))

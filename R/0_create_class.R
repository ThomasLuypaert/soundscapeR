...
setMethod(
  ...
  function(object) {
    ...
    cat(
      "    Sunrise time at sampling point: ",
      as.character(hms::as_hms(round(object@sunrise))),
      "\n"
    )
    cat(
      "    Sunset time at sampling point: ",
    ...
    if (object@aggregated_df[1, 1] == "missing") {
      if (object@binarized_df[1, 1] == "missing") {
        ...
        cat(crayon::red(
          "    It appears your next step is: 'aggregate_df()'",
          "\n"
        ))
      }
    } else {
      ...
      cat("\n")
      cat(
        "    Output format: ", object@output,
        "\n"
      )
      cat(
        "    Data frame frequency range: ",
        min(
          as.numeric(
            rownames(

...
#' @export
#'
ss_aggregate <- function(binarized_soundscape,
                         output = "incidence_freq"){
  ...
  assertthat::on_failure(test_2) <- function(call, env){
    ...
    paste0(deparse(call$x), " is not an S4-object of the type 'soundscape'. Please supply the binarized_soundscape object produced by the ss_binarize() function. Consult the package documentation for further information.")

  }

  assertthat::on_failure(test_3) <- function(call, env){

    paste0(deparse(call$x), " is an empty S4-object of the type 'soundscape'. Did you supply the binarized_soundscape argument produced using the ss_binarize() function? If so, something has gone wrong, please re-run the ss_binarize() function.")
  ...
  assertthat::on_failure(test_10) <- function(call, env){
    ...
    paste0(deparse(call$x), " is not a single positive integer. Did you supply the binarized_soundscape argument produced using the ss_binarize() function? If so, something has gone wrong, please re-run the ss_binarize() function, and pay special attention to the samplerate and window arguments.")
  ...
  assertthat::on_failure(test_14) <- function(call, env){
    ...
    paste0(deparse(call$x), " is not NA. Did you supply a post-binarization or post-aggregation soundscape to the ss_threshold_check() function? Please supply the output of the ss_index_merge() function to this argument.")
  ...
  assertthat::on_failure(test_15) <- function(call, env){
    ...
    paste0(deparse(call$x), " is not a valid data frame. It is possible the argument is not a data frame, is empty, or contains NA/non-numeric values. Did you supply the binarized_soundscape argument produced using the ss_binarize() function? If so, something has gone wrong, please re-run the ss_binarize() function.")
  ...
  assertthat::on_failure(test_18) <- function(call, env){
    ...
    paste0(deparse(call$x), " has values smaller than 0 or greater than 1. The function expects a binary data frame which is the output of the binarization step using the ss_binarize() function.")
  ...
  assertthat::on_failure(test_19) <- function(call, env){
    ...
    paste0(deparse(call$x), " is not a missing data frame. Did you supply a post-aggregation soundscape to the binarized_soundscape argument of the ss_binarize() function? Please supply the output of the ss_binarize() function to this argument.")
  ...
  assertthat::on_failure(test_20) <- function(call, env){
    ...
    paste0(deparse(call$x), " is not a list of NAs. Did you supply a post-aggregation soundscape to the binarized_soundscape argument of the ss_binarize() function? Please supply the output of the ss_binarize() function to this argument.")
